		...
		<view class="my-order">
			...
			<view class="nav acea-row row-around">
				<view class="item" :class="orderStatus == 10 ? 'on' : ''" @click="statusClick(10)">
					<view>全部</view>
					<view class="num">{{ allCount || 0 }}</view>
				</view>
					...
					<view @click="goOrderDetails(item.order_id)">
						...
						<view class="item-info acea-row row-between row-top" v-for="(item, index) in item.cartInfo" :key="index">
							...
							<view class="text acea-row row-between">
								<view class="name line2">{{ item.productInfo.k_color }}</view>
								<view class="money">
...
export default {
	...
	computed: {
		...mapGetters(['isLogin']),
		allCount(){
			let all = Number(this.orderData.unpaid_count)+Number(this.orderData.unshipped_count)+Number(this.orderData.received_count)+Number(this.orderData.complete_count)
			return all
		}
	},
	onShow() {
