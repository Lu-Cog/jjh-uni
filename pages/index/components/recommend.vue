<template>
	<view class="productList">
		<view class='list acea-row row-between-wrapper' v-if="isShow && goodList.length">
			<view class='item' v-for="(item,index) in goodList" :key="index" @click="goDetail(item)">
				<view class='pictrue'>
			...
			<view class="more" @click="gotoList">查看更多</view>
		</view>
		<view class='list acea-row row-between-wrapper' v-if="!isShow && isIframe && goodList.length">
			<view class='item' v-for="(item,index) in goodList" :key="index" @click="goDetail(item)">
				<view class='pictrue'>
					<image :src='item.image'></image>
					<span class="pictrue_log_class pictrue_log_big" v-if="item.activity && item.activity.type === '1'">秒杀</span>
		...
		<block v-if="isIframe && !goodList.length">
...
<script>
	...
	export default {
		...
		props: {
			...
			authType:{
				type:Number,
				default:uni.getStorageSync('authType')
			}
		},
		components: {
			goodLists
		...
		methods: {
			gotoList(){
				uni.navigateTo({
					url:'/pages/goods_list/index?title='+this.zcInfo.title+'&zc='+this.zcInfo.zcid
				})
			},
			// 产品列表
