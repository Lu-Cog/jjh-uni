<template>
	<view class="swiperBg" :style="{marginTop:mt+'px'}">
		<block v-if="isShow && dataConfig.length">
			<view class="swiper square" v-if="dataConfig.length">
				<swiper indicator-dots="true" :autoplay="true" :circular="circular" :interval="interval" :duration="duration"
				 indicator-color="rgba(255,255,255,0.6)" indicator-active-color="#fff" @change='getSwiperIndex'>
					<block v-for="(item,index) in dataConfig" :key="index">
						<swiper-item>
		...
		<block v-if="!isShow && isIframe && dataConfig.length">
			<view class="swiper square" v-if="dataConfig.length">
				<swiper indicator-dots="true" :autoplay="true" :circular="circular" :interval="interval" :duration="duration"
				 indicator-color="rgba(255,255,255,0.6)" indicator-active-color="#fff" @change='getSwiperIndex'>
					<block v-for="(item,index) in dataConfig" :key="index">
		...
		<block v-if="isIframe && !dataConfig.length">
...
<script>
	...
	export default {
		...
		created() {
			// #ifdef MP
			this.mt = parseFloat(statusBarHeight) + 50
			// #endif
		...
		methods: {
			...
			getSwiperIndex(e){
				...
				this.$emit('swiperIndex',currentIndex)
			},
			goDetail(url) {
				goPage().then(res => {
