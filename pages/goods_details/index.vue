	...
	<view class="product-con">
		<view class="navbar" :style="{ height: navH + 'rpx', opacity: opacity }">
			<view class="navbarH" :style="'height:' + navH + 'rpx;'">
				<view class="navbarCon acea-row row-center-wrapper" :style="{ padding:'0 '+ navbarRight + 'px' }">
		...
		<view>
			<scroll-view :scroll-top="scrollTop" scroll-y="true" scroll-with-animation="true" :style="'height:' + height + 'px;'" @scroll="scroll">
				<view id="past0">
					...
					<view class="defaultP">
						<view>基础工费：<text>{{authType>1?storeInfo.fee_jcgj:'???'}}元/g</text></view>
						<view>附加费：<text>{{authType>1?storeInfo.fee_fjf:'???'}}元/g</text></view>
					</view>
					<view class="wrapper">
						<view class="introduce" v-text="storeInfo.store_name"></view>
						<view class="share acea-row row-between row-bottom">
							<view class="money font-color" v-if="authType>1">
								￥
								<text class="num" v-text="storeInfo.price || 0"></text>
								<text v-if="storeInfo.spec_type">起</text>
							</view>
							<view class='noAuth' v-else>
								<text>￥???</text><text>认证查看批发价</text>
							</view>
							<view class="iconfont icon-fenxiang" @click="listenerActionSheet"></view>
						</view>
						<view class="label">
							款号：{{storeInfo.kh_code}}
						</view>
						<view class="label acea-row row-between-wrapper" style="padding-bottom: 20rpx;">
							<view v-text="'成色:' + (storeInfo.k_color || 0)"></view>
							<view v-text="'金重:' + (storeInfo.g_weight_1)+'-'+(storeInfo.g_weight_2) + (storeInfo.unit_name || '')"></view>
							<view v-text="'库存:' + (storeInfo.stock || 0)"></view>
						</view>
				...
				<view class="product-intro" id="past3">
					...
					<image src="https://jjhzb-img.iqweb.net/attach/2022/12/286e6202212261748342291.jpg" mode="widthFix"></image>
					<image src="https://jjhzb-img.iqweb.net/attach/2022/12/c6a96202212261748408279.jpg" mode="widthFix"></image>
					<image src="https://jjhzb-img.iqweb.net/attach/2022/12/ed6d7202212261752096961.jpg" mode="widthFix"></image>
					<view class="conter_text">
						<view class="title">
							如何保养
						</view>
						<ul>
							<li>*避免化妆品、香水、发胶等酸性物质接触金饰，影响光泽，出现发白变红等现象。</li>
							<li>*避免游泳时佩戴金饰，因泳池中的水经过加氯处理，会令金饰氧化变得暗哑无光泽。</li>
							<li>*避免大量流汗时佩戴金饰，因人体汗液会与黄金发生化学反应，令金饰出现黑色斑点，影响美观。</li>
						</ul>
						<view class="title">
							如何清洁
						</view>
						<ul>
							<li>*把金饰放入清水稀释后的中性洗液中，用软布或毛刷清表面尘埃污垢。</li>
							<li>*利用水柱的冲击力清理污垢，清洁后用软棉布抹千或自然风干。</li>
							<li>*定期将金饰送至珠宝店清洁以恢复光彩。</li>
						</ul>
					</view>
					<image src="https://jjhzb-img.iqweb.net/attach/2022/12/3155b202212261752094948.jpg" mode="widthFix"></image>
					<view class="conter_text">
						<ul>
							<li>*商品质量问题、破损、错发等非买家原因退货，平台包邮。</li>
							<li>*质量问题、错发售后申请，24小时内审核完成。</li>
							<li>*珠宝属于贵重物品，一旦出库，产品无质量问题暂不退换货品。</li>
						</ul>
					</view>
				</view>
			</scroll-view>
		...
		<view class="footer acea-row row-between-wrapper">
			...
			<navigator open-type="reLaunch" class="animated item" :class="animated == true ? 'bounceIn' : ''" url="/pages/order_addcart/order_addcart" hover-class="none">
				...
				<view class="p_center">购物袋</view>
			</navigator>
			<view class="bnt acea-row" v-if="attr.productSelect.stock <= 0">
				<form @submit="joinCart" class="joinCart bnts"><button class="joinCart bnts" form-type="submit">加入购物袋</button></form>
	...
	methods: {
		...
		getUserInfo: function() {
			...
			getUserInfo().then(res => {
				that.authType = res.data.auth_type
				uni.setStorageSync('authType',that.authType)
		...
		attrVal(val) {
			if(this.attr.productAttr[val.indexw].index == this.attr.productAttr[val.indexw].attr_values[val.indexn]){
				this.$set(this.attr.productAttr[val.indexw], 'index', '');
			}else{
				this.$set(this.attr.productAttr[val.indexw], 'index', this.attr.productAttr[val.indexw].attr_values[val.indexn]);
			}
		},
		attrVals(val) {
			if(this.attr.productValues[val].index){
				this.$set(this.attr.productValues[val], 'index', 0);
			}else{
				this.$set(this.attr.productValues[val], 'index', 1);
			}
		},
		/**
		 * 属性变动赋值
		...
		ChangeAttr: function(res) {
			...
			if(this.attr.productAttr.length>1){
				return this.getCheckedValueNo(res)
			}
		...
		// 没选全的时候没选择
		getCheckedValueNo: function(value) {
			if(!value){
				return this.attr.productValues = this.productValue
			}
			let productAttr = this.attr.productAttr;
			// 没选中的属性数组
			let noValue = [];
			this.attr.productValues = []
			// 没选中的属性
			for (let j = 0; j < productAttr[1].attr_values.length; j++) {
				noValue.push(productAttr[1].attr_values[j]);
			}
			for (let s = 0; s < noValue.length; s++) {
				let noAttrValue = value+","+noValue[s]
				// 查找
				let productSelect = this.productValue[noAttrValue]
				console.log(productSelect,'productSelect');
				if(productSelect){
					
					this.attr.productValues.push(productSelect)
				}
			}
		...
		getGoodsDetails: function() {
			...
			getProductDetail(that.id)
				.then(res => {
					...
					that.$set(that.attr, 'k_color', storeInfo.k_color);
					...
					that.$set(that, 'routineContact', res.data.routine_contact_type);
					...
					// #endif
					that.getCartCount();
					if(res.data.productValue.length<=0){
						that.DefaultSelect();
					}else{
		...
		DefaultSelect: function() {
			let productAttr = [{
				...
				cart_num:1,
				cost:this.storeInfo.cost || '',
				ot_price:this.storeInfo.ot_price
			}];
			this.$set(this.attr, 'productValues', productAttr);
		...
		joinCart: function(e) {
			...
			if (this.isLogin === false) {
				...
				uni.showModal({
					title:'温馨提示',
					content:'平台只做批发，谢绝零售！所有用户需通过认证后，才可下单拿货，请点击"确定"去认证！',
					success(res) {
						if(res.confirm){
							uni.navigateTo({
								url:'/pages/auth/auth'
		...
		goCat: function(news) {
			let productSelects = []
			for(let i in this.attr.productValues){
				if(this.attr.productValues[i].index==1){
					productSelects.push(this.attr.productValues[i])
				}
			}
			let that = this,
				productSelect = productSelects || [that.productValue[this.attrValue]];
			//打开属性
			if (that.attrValue) {
			...
			//如果有属性,没有选择,提示用户选择
			if (productSelect.length <= 0 && that.isOpen === true)
				return that.$util.Tips({
					title: '请选择规格'
				});
			let unique = []
