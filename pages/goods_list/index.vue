<template>
	<view>
		<view class='productList'>
			...
			<view class="navMain">
				<view class='nav acea-row row-middle'>
					<view class='item line1 font-color'>综合</view>
					...
					<view class='item' :class='xiaol ? "font-color":""' @click='set_where(5)'>销量</view>
					<!-- down -->
					<view class='item' @click='orderbyChange'>筛选</view>
				</view>
			...
			<view class='list acea-row row-between-wrapper' :class='is_switch==true?"":"on"'>
				<view class='item' :class='is_switch==true?"":"on"' hover-class='none'
					...
					<view class='text' :class='is_switch==true?"":"on"'>
						...
						<view class='defutP row-between-wrapper'>
							<view>基础工费：<text>{{authType>1?item.fee_jcgj:'???'}}元/g</text></view>
							<view>附加费：<text>{{authType>1?item.fee_fjf:'???'}}元/g</text></view>
						</view>
						<view class='vip acea-row row-between-wrapper'>
							<view>克重：{{item.g_weight_1}}-{{item.g_weight_2}}g</view>
							<view>库存：{{item.stock}}</view>
						</view>
						<view class='money font-color' v-if="authType>1">￥<text class='num'>{{item.price}}</text></view>
		...
		<view class='noCommodity' v-if="productList.length==0 && where.page > 1">
			...
			<recommend :authType='authType' :hostProduct="hostProduct"></recommend>
		</view>
		<fjj-condition ref='condition' @touchmove.stop :color="color" :list="menuList" :defaultValue="defaultValue"
			@result="resultConditon" @sidList='sidList'/>
	</view>
</template>
...
<script>
	...
	export default {
		...
		onLoad: function(options) {
			this.authType = uni.getStorageSync('authType')
			this.cateSelect()
			if (options.title) {
			...
			this.where.pl = options.pl || 0;
			this.where.b = options.b || 0;
			this.where.zc = options.zc || 0;
		...
		methods: {
			...
			cateSelect() {
				cateSelect().then(res => {
					...
					this.menuList.push({
						...
						'key': 'sid',
						'isMutiple': false, //单选
						'detailList':res.data.cate[0].children,
					})
					this.menuList.push({
						...
						'key': 'color',
						'isMutiple': false, //单选
						'detailList':res.data[1].sons,
					})
					this.menuList.push({
						'title': res.data[2].name,
						'type': 'custom',
						'key': 'style',
						'isMutiple': false, //单选
						'detailList':res.data[2].sons,
					})
					this.menuList.push({
						...
						'key': 'gg',
						'isMutiple': false, //单选
						'detailList':res.data[3].sons,
					})
					this.menuList.push({
						'title': res.data[4].name,
						'type': 'custom',
						'key': 'art',
						'isMutiple': false, //单选
						'detailList':res.data[4].sons,
					})
					this.menuList.push({
						'title': res.data[5].name,
						'type': 'custom',
						'key': 'ks',
						'isMutiple': false, //单选
						'detailList':res.data[5].sons,
					})
					this.menuList.push({
						'title': res.data[6].name,
						'type': 'custom',
						'key': 'model',
						'isMutiple': false, //单选
						'detailList':res.data[6].sons,
					})
			...
			resultConditon(obj) {
				...
				this.get_product_list(true,obj.str_result);
			},
			orderbyChange(obj) {
			...
			//查找产品
			get_product_list: function(isPage,data) {
				let that = this;
				that.setWhere();
...
<style scoped lang="scss">
	.navMain {
		...
		.item_c {
			...
			width: 100%;
			z-index: 10;
			box-shadow: 0 40rpx 40rpx 20rpx rgba(0, 0, 0, 0.2);
