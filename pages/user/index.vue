				...
				<view class="user-info">
					...
					<view class="info">
						...
						<view class="name" v-if="userInfo.uid">
							...
							<view v-if="userInfo.auth_type>1" class="vip-txt" @click="gotoAuth"><image class="vip" src="@/static/images/yesAuth.png"></image>已认证</view>
						...
						<view class="num" v-if="userInfo.level_tip">
							<view class="num-txt">会员等级：{{userInfo.level_tip}}</view>
						</view>
		...
		<view class="main">
			...
			<view class="order-wrapper">
				<view class="order-hd flex">
					...
					<navigator class="right flex" hover-class="none" url="/pages/users/order_list/index?status=10" open-type="navigate">
						查看全部
			...
			<!-- 会员菜单 -->
			<view style="margin-top: 20rpx; border-radius: 6rpx; overflow: hidden;">
				...
				<view class="user-menus">
					...
					<!-- #ifdef MP -->
					<template v-for="(item, index) in MyMenus">
						<view v-if="item.url !== '#' && item.url !== '/pages/service/index' && item.url!='/pages/customer_list/chat' || (item.url=='/pages/customer_list/chat' && routineContact == '0')" class="item" :key="index" @click="goMenuPage(item.url, item.name)">
							<image :src="item.pic"></image>
							<text>{{ item.name }}</text>
						</view>
					</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getWechatuserinfo() {
				//#ifdef H5
				uni.setStorageSync('snRouter','/pages/user/index');
			...
			getUserInfo: function() {
				...
				getUserInfo().then(res => {
					...
					uni.setStorageSync('authType',res.data.auth_type)
					...
					that.orderMenu.forEach((item, index) => {
						switch (item.title) {
							...
							// 	break
							case '已完成':
								item.num = res.data.orderStatusNum.complete_count
								break
						}
					})
				});
								...
								Routine.authUserInfo(userInfo)
									.then(res => {
										if (res.data.key !== undefined && res.data.key) {
											...
											uni.setStorageSync('authKey',res.data.key)
			...
			getMyMenus: function() {
				...
				getMenuList().then(res => {
					...
					this.routineContact = res.data.routine_contact_type
				});
			},
			// 编辑页面
			goEdit() {
			...
			goMenuPage(url, name) {
				if (this.isLogin) {
					if (url.indexOf('http') === -1) {
						...
						if (name && name === '客服接待') {
							return uni.navigateTo({
								url: `/pages/annex/web_view/index?url=${HTTP_REQUEST_URL}${url}`
							});
						}
						// #endif
						uni.navigateTo({
