					...
					<view v-if="item.type === 'rangenumber'">
						...
						<view class="dateContent rangenumber-content flex">
							<view class="rangenumber-input">
								<input class="m-input" type="digit" clearable v-model="result[item.minName || (item.key + 'Min')]"
									:placeholder="item.minPlaceholder || '最小值'"
									@blur="numberInputBlur(item)"></input>
							</view>
							<text>-</text>
							<view class="rangenumber-input">
								<input class="m-input" type="digit" clearable v-model="result[item.maxName || (item.key + 'Max')]"
							...
							if (list[i].key === key) {
								if (list[i].type === 'custom') {
									list[i].detailList.map((item, index) => {
										if (Array.isArray(value[key]) && value[key].indexOf(item.id) > -1) {
											item.isSelected = true;
										} else if (value[key] == item.id) {
											item.isSelected = true;
										}
										return item;			
									})
			...
			itemTap(index, list, key, isMutiple) {
				if (isMutiple == true) {
					...
					if (list[index].isSelected) {
						this.result[key].push(list[index].id);
					} else {
						list[index].isSelected = false;
						var idx = this.result[key].indexOf(list[index].id);
					...
					if(key=='cid'){
						this.$emit("sidList", list[index],this.result);
					}
				}
				// #ifdef H5 || APP-PLUS
				this.$forceUpdate();
			...
			sureClick() {
			    this.menuList.forEach(menu => {
					...
					if (menu.type == 'custom' && list.length > 0) {
						...
						list.forEach(item => {
							if (item.isSelected) {
								selectedValueList.push(item.id);
							}
						});
