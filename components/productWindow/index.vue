<template>
	<view>
		<view class="product-window" :class="(attr.cartAttr === true ? 'on' : '') + ' ' + (iSbnt?'join':'') + ' ' + (iScart?'joinCart':'')">
			...
			<view class="rollTop">
				<view class="productWinList" v-if="attr.productAttr.length>1">
					<view class="item acea-row row-middle" v-for="(item, indexw) in attr.productAttr" :key="indexw" v-if="indexw==0">
				...
				<view class="productValueList">
					<view class="productItem" :class="item.index === 1 ? 'on' : ''" v-for="(item,index) in attr.productValues" :key="index" @click="tapAttrs(index)">
						<view class="title">
							{{attr.k_color}} (条码：{{item.unique}})，重量：{{item.weight}}g，附加工费：￥{{item.volume}}/g
						</view>
						<view class="price">
							￥{{item.price}}
						</view>
						<view class="otherPrice">
							金料：￥{{item.cost}}  工费：￥{{item.ot_price}}
						</view>
						<image src="../../static/images/duigou.png" mode="widthFix"></image>
					</view>
				</view>
...
<script>
	export default {
		...
		methods: {
			...
			tapAttrs:function(index){
				let that = this;
				that.$emit("attrVals",index);
			},
			tapAttr: function(indexw, indexn) {
				...
				if(this.attr.productAttr[indexw].index == this.attr.productAttr[indexw].attr_values[indexn]){
					this.$set(this.attr.productAttr[indexw], 'index', '');
				}else{
