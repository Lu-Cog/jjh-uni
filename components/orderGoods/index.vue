			...
			<view class='item acea-row row-between-wrapper' v-for="(item,index) in cartInfo" :key="index" @click="jumpCon(item.product_id)">
				...
				<view class='text'>
					<view class='acea-row row-between-wrapper'>
						<view class='name line1'>{{item.productInfo.k_color}}</view>
						<view class='num'>x {{item.cart_num}}</view>
					</view>
					<view class='attr' v-if="item.productInfo.attrInfo">金重：{{item.productInfo.attrInfo.weight}}g，款号：{{item.productInfo.kh_code}}，金料：￥{{item.productInfo.attrInfo.cost}}，工费：￥{{item.productInfo.attrInfo.ot_price}}</view>
